...
/*----------------------------------------------------------------------------*/
void RequestICMP(mtcp_manager_t mtcp, uint32_t saddr, uint32_t daddr,
...
int ProcessICMPPacket(mtcp_manager_t mtcp, struct iphdr *iph, int len)
{
	...
	int i;
	int to_me = FALSE;
	// printf("[+] hl ProcessICMPPacket iph->daddr(%#x)\n", iph->daddr);
	/* process the icmp messages destined to me */
	for (i = 0; i < CONFIG.eths_num; i++)
	{
		HL_PRINT("[+] hl ProcessICMPPacket iph->daddr(%#x),config(%#x)\n", iph->daddr, CONFIG.eths[i].ip_addr);
