...
#define MAX_SACK_ENTRY 8
	...
	TAILQ_ENTRY(tcp_stream)
	he_link; /* hash table entry link */
...
struct tcp_send_vars
{
	...
	TAILQ_ENTRY(tcp_stream)
	timer_link; /* timer link (rto list, tw list) */
	TAILQ_ENTRY(tcp_stream)
	timeout_link; /* connection timeout link */
...
int EqualFlow(const void *flow1, const void *flow2);
