 ...
 * but it should work.
 */
int IsPciEnt(const struct dirent *entry)
{
 ...
 */
int localversionsort(const void *elem1, const void *elem2)
{
...
/*--------------------------------------------------------------------------*/
int probe_all_rte_devices(char **argv, int *argc)
{
	...
	for (j = 0; j < 3; j++)
	{
		...
		for (i = 0; i < total_files; i++, pci_index++)
		{
			...
			if (argv[*argc] == NULL ||
				...
				goto alloc_err;
			*argc += 2;
			if (sscanf(dirlist[i]->d_name, PCI_DOM ":" PCI_BUS ":" PCI_DEVICE "." PCI_FUNC,
